...
| GitHub Actions OIDC CI/CD | **built and applied 2026-07-06** — provider + per-branch roles + EKS access entries. Three workflows: `terraform.yml` (plan-on-PR, **verified end-to-end against live `chess-prod`, all units succeed**), `bootstrap-shared.yml`/`bootstrap-prod.yml` (dispatch-only, no-plan cold apply, neither run yet). Also fixed along the way: cross-platform S3 state key bug, Helm `--set` comma-parsing bug. See GitHub Actions CD section |
...
- Two IAM roles, trust policy scoped per branch via the `sub` claim: `chess-shared-cicd` (`ref:refs/heads/dev`) and `chess-prod-cicd` (`ref:refs/heads/main`) — a `workflow_dispatch`/`push` run on one branch can never assume the other's role. Both roles **also** accept `sub = "repo:<repo>:pull_request"` — GitHub sets this fixed value (no branch info at all) for every `pull_request`-triggered run regardless of base/head branch, which is the only way the `plan` job (below) can assume either role at all. Acceptable gap for now since both roles carry identical permissions and only `plan` (read-only) runs on `pull_request` — revisit with a `base_ref`-based condition if the two roles' permissions ever diverge.
- Permissions: one inline policy per role (not attached managed policies — IAM's `PoliciesPerRole` quota is 10, and this needs 19 action prefixes; inline policies don't count against that quota). Service-scoped (`ec2:*`, `eks:*`, `autoscaling:*`, `kms:*`, `logs:*`, `rds:*`, `elasticache:*`, `s3:*`, `cloudfront:*`, `acm:*`, `route53:*`, `elasticloadbalancing:*`, `iam:*`, `ssm:*`, `sqs:*`, `events:*`, `wafv2:*`/`waf:*`/`waf-regional:*` — the last three for planned-but-not-yet-built WAF), not `AdministratorAccess` — verified against every `resource "aws_*"` in every module, what the community VPC/EKS/Karpenter/IRSA modules provision, and AWS's own managed-policy docs (no managed policy exists for EKS cluster-lifecycle actions at all — `AmazonEKSClusterPolicy` and siblings are for the cluster/node's own service role, not a caller of `eks:CreateCluster`).

**Missing EKS-adjacent permissions (found and fixed 2026-07-06):** the first real `plan` run against the live `chess-prod` cluster (after the S3 key fix below let it read real state at all) failed on `kms:DescribeKey` and `logs:DescribeLogGroups` — describing an EKS cluster also describes its KMS key (envelope encryption) and its CloudWatch Logs group (control plane logging), neither of which is an EKS-specific action. Rather than keep discovering these one failure at a time, cross-checked the whole permission set against `terraform-aws-modules/terraform-aws-eks`'s own `iam-permissions.md` (the module's maintainers' documented list of what a principal needs to create/manage a cluster with this module) — that turned up `autoscaling:*` too (managed node group / ASG operations), added preemptively before it caused its own failure. All three (`autoscaling:*`, `kms:*`, `logs:*`) are in the permission list above now.
...
**Cross-platform S3 state key bug (found and fixed 2026-07-06):** `root.hcl`'s key was `"${path_relative_to_include()}/terraform.tfstate"` — that function returns OS-native path separators, so a Windows apply wrote e.g. `environments\prod\vpc/terraform.tfstate` (backslashes) while the Linux CI runner computed `environments/prod/vpc/terraform.tfstate` (forward slashes) for the exact same unit — two different S3 objects. CI saw no state at all for anything only ever applied from a laptop and planned as if creating it from scratch. Fixed with `replace(path_relative_to_include(), "\\", "/")`; all 26 existing state objects were migrated to the normalized key (`aws s3api copy-object` + ETag verification + `delete-object`, run via **PowerShell** — an initial Git Bash attempt with `aws s3 cp` failed because MSYS mangles `s3://` URIs containing backslashes, reverted cleanly before touching any real state).

**Helm `--set` comma-parsing bug (found and fixed 2026-07-06):** ArgoCD's `configs.rbac.policy.csv` value is CSV-formatted (literally full of commas) but was being passed through `helm_release`'s `set` block — Helm's `--set` syntax reads unescaped commas in a value as separators between *additional* key=value pairs, so a real apply failed with `key " role:chess-prod-viewer" has no value`. Moved to `values` (a real YAML string, where commas mean nothing special) instead.

**Workflows — three files, not one:**

| Workflow | Trigger | Does |
|---|---|---|
| `terraform.yml` | `pull_request` into `dev`/`main` | `plan` only, per-unit (not `run --all` — see Architectural Decisions below for why), required branch-protection check, nothing applied |
| `bootstrap-shared.yml` | `workflow_dispatch`, dev branch only | `terragrunt run --all apply` against `terraform/environments/shared`, **no plan step** |
| `bootstrap-prod.yml` | `workflow_dispatch`, main branch only | Same, against `terraform/environments/prod` |

The bootstrap workflows are deliberately separate from `terraform.yml` and skip `plan` entirely: `plan` genuinely fails against a torn-down environment (units like `vpn`/`argocd`/`eso` do real AWS lookups — e.g. `vpn`'s `data "aws_eks_cluster"` — against a dependency that doesn't exist yet, getting a mock value that then 404s against the real AWS API), while a full `apply` in dependency order resolves fine — by the time it reaches `vpn`, `eks` was already created earlier in the same run. Each bootstrap workflow fails closed with an explicit `::error::` if dispatched from the wrong branch, since `workflow_dispatch` can be run from any branch a user picks. Once an environment is up, ongoing changes go back through `terraform.yml`'s plan-checked PRs.

Neither bootstrap workflow has ever been run end-to-end yet — `terraform.yml`'s `plan` job has (verified against the live `chess-prod` cluster, all units succeeded after the fixes above).

`terraform.yml`'s `plan` job is **per-unit, not `run --all`**: confirmed empirically (against a live prod cluster and locally) that `terragrunt run --all plan` substitutes mock dependency outputs for any unit sharing a batch with its dependency (e.g. `eks`), even when that dependency has real, already-applied state — breaking the kubectl/helm provider config for `nodepools`/`vpn`/`argocd`/`eso` every time, regardless of whether the cluster actually exists. A standalone `terragrunt plan` inside each unit's own directory reads real dependency state correctly instead.

`ADMIN_PRINCIPAL_ARN` is passed into every plan/apply via a GitHub Actions **secret**, not a plain repo variable — without it, the CI role's own apply would see the personal `access_entries.personal` entry missing from desired config and destroy it. Secret (not variable) for the same reason it's not committed for local applies either: pairs the account ID with a personal IAM username, more targeted than the account ID alone (already public via the state bucket name). Role ARNs are hardcoded plain values in the workflows, not secrets — a role ARN only pairs the (already-public) account ID with a role *name* chosen for it, no personal identity attached, unlike `ADMIN_PRINCIPAL_ARN`.

**Public repo, but not open to strangers:** this repo is public, but that doesn't let anyone trigger these workflows or reach the AWS role. `workflow_dispatch` can only be invoked by someone with write access to the repo (not just anyone who can read it), and `pull_request` runs from a fork don't receive `secrets.*` or an OIDC token at all (a GitHub Actions default specifically to stop a stranger's PR from exfiltrating credentials) — the `plan` job would simply fail to authenticate in that case, not silently run with real AWS access. This only holds because the workflows use plain `pull_request`, not `pull_request_target` (which would grant base-repo secrets even to fork PRs).

**Manual one-time setup** (not something Terraform or the workflows do for you):
...
1. **CloudFront's 30s default origin-read-timeout** — plausible first guess (idle game = no data on the socket for a while), but the disconnect happened *immediately*, not after ~30s, so this wasn't it. (Tried raising `origin_read_timeout`/`origin_keepalive_timeout` anyway on general principle — see the apply-time follow-up below for why that didn't end up sticking.)
...
- `InvalidOriginReadTimeout: ... not within the valid range` on the `frontend` apply, **twice** — first at `origin_read_timeout = 180` (per AWS's own documented "up to 180s without a support request" guidance), then again at `60`. Checked the authoritative source (AWS's `CustomOriginConfig` API reference, not a blog/summary) to rule out a documentation-vs-reality mismatch: the API's own hard schema range is 1–120s for `OriginReadTimeout` (1–300s for `OriginKeepaliveTimeout`) — so 60 should have been valid by the API's own published constraints. Since it wasn't, this account carries a separate, lower, account-specific quota holding it at the literal default with no self-service headroom above it — not visible via the Service Quotas API either (`list-service-quotas --service-code cloudfront` returns nothing for this account). Reverted to leaving both fields unset (CloudFront's hard defaults: 30s response / 5s keep-alive) rather than guess a third value — raising this for real needs an actual AWS Support quota-increase request, not a Terraform change.
...
**Known gap, not fixed here (application-level, not infra):** confirmed by reading the code that game-service has no periodic WebSocket ping/keepalive at all — every message is sent only in response to an actual event (move, resign, connect/disconnect), never on a timer, and neither FastAPI/Starlette nor the frontend do this automatically either. Combined with CloudFront's 30s default origin-read-timeout (now unchanged from default, see above), a sufficiently long pause with no moves and no other traffic on the socket risks a CloudFront-initiated disconnect independent of anything ALB/nginx-side — arguably more exposed now than if a quota increase had gone through. The durable fix is an application-level periodic ping (e.g. every 20–30s) from either end — flagged for the game-service/frontend teams, out of scope for this repo.
