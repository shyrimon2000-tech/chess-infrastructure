...
- **The ALB is reachable from nowhere except CloudFront — two independent layers, not one.** By default the controller auto-creates a permissive security group open to the internet on the listener ports. Instead, `terraform/modules/alb-controller` creates its own `aws_security_group.alb`, referenced by the Ingress via `alb.ingress.kubernetes.io/security-groups` (matched by the SG's `Name` **tag**, not its AWS-generated `groupName` — easy to get wrong), which bypasses that auto-creation. `manage-backend-security-group-rules: "true"` keeps the controller still auto-managing the ALB→pod backend rules despite the custom frontend SG.
  1. **Network layer** — ingress restricted to AWS's managed prefix list `com.amazonaws.global.cloudfront.origin-facing` (port 443 only, matching the ALB's HTTPS-only listener — see above). Authoritative, AWS-maintained source for "which IPs does CloudFront originate from" — hand-maintaining that CIDR list would go stale as AWS rotates edge IPs.
...
- **Depends on `vpc` (for apply-time parallelism) and, separately, `vpn` (ordering-only, for destroy correctness).** Originally depended only on `vpc`, deliberately not `eks`, so RDS provisioning (storage allocation, Multi-AZ standby setup, DNS propagation — 10-15+ minutes regardless of what else is happening) could run in parallel with `eks`/`karpenter`/`nodepools` instead of queuing behind the cluster. The `vpn` dependency was added afterward for a different reason: this module's `mysql_database`/`mysql_user`/`mysql_grant` resources (the `mysql` provider, see next bullet) need a live TCP:3306 connection to RDS on **both** apply and destroy, not just apply. Without an explicit edge to `vpn`, `terragrunt run --all destroy` had no reason to keep `vpn` alive until `rds` finished — it could tear `vpn` down first or in parallel, stranding those `mysql`-provider resources with no path left to reach RDS and drop them. Terragrunt destroys in reverse dependency order, so this dependency (mock outputs never read — ordering only, same pattern as `eso → nodepools`) makes `rds` (and `elasticache`, which already depends on `rds`) destroy *before* `vpn`, not after. **Trade-off, accepted deliberately:** `vpn` itself depends on `eks`, so `rds` now transitively depends on `eks` too — the original apply-time parallelism is gone. Correctness on destroy was judged more important than shaving minutes off apply.
