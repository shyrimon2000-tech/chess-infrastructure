...
1. **CloudFront's 30s default origin-read-timeout** — plausible first guess (idle game = no data on the socket for a while), but the disconnect happened *immediately*, not after ~30s, so this wasn't it. (Still raised `origin_read_timeout`/`origin_keepalive_timeout` to 180s/60s on general principle — worth having regardless, just not the cause here.)
...
- `terraform/modules/alb-controller/main.tf` — added a second security-group ingress rule for port 8443, same CloudFront-managed prefix list as port 443 (no broader network exposure — same trust boundary, just one more port).
